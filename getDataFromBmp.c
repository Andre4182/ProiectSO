...
void convert_to_grayscale(char * file, int width, int height) {
...
void read_bmp_header(char * file_path) {
                ...
                if (currentEntry -> d_type == DT_REG || currentEntry -> d_type == DT_LNK || currentEntry -> d_type == DT_DIR) {
                    ...
                    if (currentEntry -> d_type == DT_REG && strstr(filePath, ".bmp")) {
                        ...
                        if (pid < 0) {
                            ...
                            convert_to_grayscale(filePath, info_header.width, info_header.height);
                        } else {
				printf("--\n");
                            int status;
                            pid_t wpid;
